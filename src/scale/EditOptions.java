...
public class EditOptions extends ProxyAutomobile implements Runnable { //create a  helper class
	...
	public void ops() {
		...
		/*case 3: 
			//t.sleep(1000);
			h.updateOptionSetNameUnsyncSleep(this.auto); //sleep
			break;
		case 1:
			if (threadNo %2 != 0) {
				h.updateOptionSetNameSync(this.auto);
				...
				h.updateOptionSetNameUnsync(auto);	
			break;
		case 2:
			if (threadNo %2!= 0)
				h.updateOptionNameSync(auto);
			else 
				h.updateOptionNameUnsync(auto);
			break;
		case 3:
			if (threadNo %2!= 0)//if not even
				h.updateOptionPriceSync(auto);
			else 
				h.updateOptionPriceUnsync(auto);	//if even
			break;*/
		}
	}
}
